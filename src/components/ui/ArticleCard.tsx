      ...
      <Link href={article.href} className="flex flex-col h-full">
        ...
        <div className="p-5 flex flex-col h-full">
          <div className="flex items-center gap-2 mb-3 flex-wrap">
            ...
            <span key={tag} className="inline-block px-1.5 py-0.5 rounded text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200">
              {tag}
            </span>
          ))}
          {article.isPopular && (
            <span className="inline-block px-1.5 py-0.5 rounded text-xs font-bold bg-red-50 text-red-600 border border-red-100">
              人気
            </span>
          )}
        </div>
